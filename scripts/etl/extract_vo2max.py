...
def move_files_local(service_account_file, local_save_path):
    ...
    filename_pattern = re.compile(r"VO2 max \d{4}\.\d{2}\.\d{2}( \d{2}\.\d{2})? Health Connect\.csv")
...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 3:
        print("Usage: python3 extract_vo2max.py <service_account_file_path> <local_save_path>")
        sys.exit(1)
