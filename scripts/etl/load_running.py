...
table_name = "running" # Changed to running as per request

def load_single_csv_to_mysql(db_config, file_path, table_name):
    """
    Loads data from a single CSV file into the specified MySQL table ('running').
    ...
    try:
        ...
        with open(file_path, 'r', newline='', encoding='utf-8') as csv_file:
            ...
            # Define the SQL INSERT statement matching the 'running' table schema
            sql = f"""
            ...
            for row_num, row in enumerate(csv_reader, start=2): # Start counting from line 2 for errors
                ...
                try:
                    # Extract and convert data according to the 'running' table schema
                    activity_type = row[0]
                    activity_name = row[1]
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 2:
        print("Usage: python3 load_running_data.py <processed_files_base_directory>")
        sys.exit(1)
    ...
    for filename in os.listdir(processed_files_base_dir):
        # Changed to look for "RUNNING*.csv" files
        if filename.startswith("RUNNING") and filename.endswith(".csv"):
    ...
    if not files_found:
        print(f"No running CSV files (e.g., 'RUNNING*.csv') found in {processed_files_base_dir} to load.")
