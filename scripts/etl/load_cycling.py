...
table_name = "cycling" # Changed to cycling as per request

def load_single_csv_to_mysql(db_config, file_path, table_name):
    """
    Loads data from a single CSV file into the specified MySQL table ('cycling').
    ...
    try:
        ...
        with open(file_path, 'r', newline='', encoding='utf-8') as csv_file:
            ...
            # Define the SQL INSERT statement matching the 'cycling' table schema
            sql = f"""
            ...
            for row_num, row in enumerate(csv_reader, start=2): # Start counting from line 2 for errors
                # Ensure row has enough columns based on the CSV header
                if len(row) < 13: # There are 13 columns in the CSV
                    print(f"Skipping malformed row {row_num} in {os.path.basename(file_path)} (too few columns): {row}")
                    continue

                try:
                    # Extract and convert data according to the 'cycling' table schema
                    activity_type = row[0]
                    activity_name = row[1]
                    activity_date = row[2] # Kept as VARCHAR(50)
                    activity_time = row[3] # Kept as VARCHAR(50)

                    # Safely convert numeric fields, defaulting to NULL or 0 if conversion fails
                    elapsed_time_seconds = int(float(row[4])) if row[4] else None # Convert potential float string to int
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 2:
        print("Usage: python3 load_cycling_data.py <processed_files_base_directory>")
        sys.exit(1)
    ...
    for filename in os.listdir(processed_files_base_dir):
        # We are looking for files specific to cycling, e.g., "CYCLING*.csv"
        # Adjust this condition if you have a different naming convention for cycling files
        if filename.startswith("CYCLING") and filename.endswith(".csv"):
    ...
    if not files_found:
        print(f"No cycling CSV files (e.g., 'CYCLING*.csv') found in {processed_files_base_dir} to load.")
