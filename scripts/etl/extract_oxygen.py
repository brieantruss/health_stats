...
from drive_config import FOLDER_OXYGEN as DRIVE_FOLDER_ID
...
def move_files_local(service_account_file, local_save_path):
    ...
    filename_pattern = re.compile(r"Oxygen saturation \d{4}\.\d{2}\.\d{2}( \d{2}\.\d{2})? Health Connect\.csv")
...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 3:
        print("Usage: python3 extract_oxygen.py <service_account_file_path> <local_save_path>")
        sys.exit(1)
